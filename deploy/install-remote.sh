...
if [ -f "$BD_PREFIX/etc/tls/server.crt" ] && command -v openssl >/dev/null 2>&1; then
  ...
  if [ -n "$crt_subj" ] && [ "$crt_subj" = "$crt_iss" ]; then
    ...
    echo "         · 安卓客户端已支持构建期注入，不需要用户在设备上装任何东西："
    echo "             ./gradlew assembleDebug -PbaidiApiBase=https://${PUBLIC_HOST} \\"
    echo "                 -PbaidiControlCa=<这张 server.crt 的本地路径>"
    echo "           一份材料同时喂 WebView（生成 network_security_config）与数据面"
    echo "           （baidimobile.Config.controlCaPEM）；不传就只信系统信任库，构建日志会当面说明。"
    echo "         · 桌面端 / 其它平台：逐端导入系统或应用的信任库。"
