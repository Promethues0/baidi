...
.DESCRIPTION
    Windows 这条链路的实机证据截至 2026-08-21（Windows 11 ARM64 一台真机）：
    阶段 A 7/7 通过、阶段 B 已跑通（建出 baidi0）、阶段 C 两次走到
    UAC → 拉起 baidi-tun → 换到敲门令牌 → 建卡，但尚未完整通过；
    NRPT 分离式 DNS 与隧道端到端仍未实测，x64 全部未实机。产物因此仍标 UNVERIFIED
    且刻意不进下载中心。这个脚本就是用来把余下的「未验证」逐条变成「已验证」或
