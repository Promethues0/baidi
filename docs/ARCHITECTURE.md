...
| **认证响应时延** | PERF-03（P1） | 登录链路零埋点。LDAP 缺省 5s 连接 + 10s 请求（api 层从不设这两个字段 → 恒取默认），OIDC 10s，而登录 handler **没有整体 deadline**（`ReadHeaderTimeout` 管不到 handler 内部）——一次外部目录登录最坏可挂 ~15s，是 PRD 那个 3s 的五倍 |
