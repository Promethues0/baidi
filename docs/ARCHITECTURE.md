...
- **管理员令牌拉不走备份**，网关/组网证书同样拉不走，反过来备机证书也调不到 `/api/v1/gateways/*`。一份备份 = CA 私钥 + 三把签名私钥 + 审计链密钥 + 认证源凭据 + IPSec PSK + 整个库；挂在 Bearer 上意味着一次令牌泄露等于系统被完整复制走，而现场只留下一条看起来很正常的「导出配置备份」审计。反向那道闸同样必要：备机若能注册成网关，剖面会把这台不转发流量的机器当可用落点下发给终端。
...
- **三态如实**：未配置备机 = `skip`（单机形态不该因为"没有备机"被扣健康分）；同步新鲜 = `pass`；落后超阈值 / 从未成功同步 / 最近一轮失败 = `warn` 且说明落后多久。逐节点阈值 = `max(全局 15 分钟, 3×备机自报间隔)` 并**封顶 6 小时**——间隔是被判定方自报的，不封顶就等于让一台自报间隔 30 天的备机永远显示新鲜。「从未成功同步」的 `lagSeconds` 是 **-1（不可判定）而不是 0**，0 的意思是"刚刚同步过"，与事实恰好相反。
