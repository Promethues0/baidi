...
// 七层 Web 代理票据走第三把密钥（隧道票据那把见下一条）：Sign 按 Use 路由到 web，且**只**持 web 公钥的一方
...
func TestWebTicketUsesItsOwnKey(t *testing.T) {
	...
	webOnly := &Keys{sess: k.web, knock: k.web, web: k.web, tunnel: k.web}
	knockOnly := &Keys{sess: k.knock, knock: k.knock, web: k.knock, tunnel: k.knock}
...
// L4 隧道身份票据走**第四把**密钥（wave11 行动 3）。
//
// ★与上一条同一条纪律，只是保护的入场路径不同——而这一条更要紧：
// 敲门令牌是每 15s 向全部落点广播的 **UDP 明文包**，截获门槛远低于隧道内的票据。
// 两者若合用一把密钥，链路上截获一次敲门包 = 拿到一张能进隧道的身份票据。
func TestTunnelTicketUsesItsOwnKey(t *testing.T) {
	k := NewTestKeys(nil, false)
	knock := k.Sign(Claims{Sub: "u", Role: "user", Name: "u", Use: UseKnock, Jti: "j"}, 90*time.Second)
	web := k.Sign(Claims{Sub: "u", Role: "user", Name: "u", Use: UseWeb, Jti: "j", Res: "oa"}, time.Minute)
	tun := k.Sign(Claims{Sub: "u", Role: "user", Name: "u", Use: UseTunnel}, 5*time.Minute)
	...
	if got := kidOfTok(tun); got != k.TunnelKid() {
		t.Fatalf("隧道票据应用 tunnel 密钥: %s", got)
	}
	for name, kid := range map[string]string{"sess": k.SessKid(), "knock": k.KnockKid(), "web": k.WebKid()} {
		if kid == k.TunnelKid() {
			t.Fatalf("★tunnel 与 %s 是同一把密钥：密钥分离没有生效", name)
		}
	}

	// 模拟只装了 tunnel 公钥的 L4 隧道口：敲门令牌与 web 票据都必须验不过。
	tunnelOnly := &Keys{sess: k.tunnel, knock: k.tunnel, web: k.tunnel, tunnel: k.tunnel}
	if _, err := tunnelOnly.Verify(tun); err != nil {
		t.Fatalf("L4 侧应能验隧道票据: %v", err)
	}
	if _, err := tunnelOnly.Verify(knock); err == nil {
		t.Fatal("★只持 tunnel 公钥时，敲门令牌必须验不过")
	}
	if _, err := tunnelOnly.Verify(web); err == nil {
		t.Fatal("★只持 tunnel 公钥时，Web 票据必须验不过")
	}
	// 反向：只装 knock 公钥的 SPA 口验不过隧道票据。
	knockOnly := &Keys{sess: k.knock, knock: k.knock, web: k.knock, tunnel: k.knock}
	if _, err := knockOnly.Verify(tun); err == nil {
		t.Fatal("★只持 knock 公钥时，隧道票据必须验不过")
	}
}
