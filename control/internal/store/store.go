...
type DefenseLine struct {
	...
	// Risk 0-100 风险分（由真实计数粗算，单调可解释，见 riskScore）。
	//
	// ★三态指针，**nil = 不可判定**（这条防线一份判定材料都没有）。改造前是 int：
	// 终端防线在「一台终端都没上报过环境」时给出 0，页面把它渲染成绿色的
	// 「0 风险分 · 良好」——那是一句没有证据的安全断言，与本项目在网关指标
	// （采不到就报不可判定，绝不补 0）与 posture 采集三态上立的纪律方向相反。
	// 那一格恰恰是全新部署与"客户端根本没装上"的常态形状。
	Risk *int     `json:"risk"` // ★不许加 omitempty：缺席会被前端 `?? 0` 补成绿色的「良好」
	Top  []string `json:"top"`  // TOP 风险实体（真实账号 / 设备，没有就是空）
	// Unknown 这条防线维度下**不可判定**的实体数：
	//   账号防线 —— 从未上报过终端环境（或判定取值认不出）的账号数；
	//   终端防线 —— 已登记进台账、却没有任何合规判定的终端数；
	//   隐身防线 —— 恒 0（攻击源是"发生过什么"的记账，没有"不可判定"这一档）。
	//
	// ★0 是有意义的取值（"确实每一个都判得出来"），故**不 omitempty**：
	// 省掉之后前端拿 `?? 0` 补回来，"没有不可判定的"与"后端根本没算这一项"就同形了。
	// 它是 TOP 为空时唯一能区分「真的没有风险实体」与「压根没有判定材料」的数。
