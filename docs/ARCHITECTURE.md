...
**本机单次样本**（2026-09-02，Apple M1 Pro 8 核 / 16 GB，macOS 27.0，go1.26.3，`go test -bench . -benchmem ./internal/proxy/`，
**不是容量承诺**）：

| 组 | 样本 | 读法 |
|---|---|---|
| 每流握手 · RSA-2048 自签（生产形态）TLS 1.3 | **5.7 ms/流**（174 流/s 串行），194 KB / 859 allocs 每流 | 这就是「每流一条 TLS」的直接成本：一个开 30 条并发连接的网页 = 30 次这个数 |
| 每流握手 · ECDSA P-256（对照） | 2.5 ms/流 | 与上行之差 ≈ 3.3 ms 是服务端 RSA 签名（Go 的常量时间 RSA 实现偏慢）——换证书算法是可量化的改进方向，但生产当前无此选项 |
| 每流握手 · 国密 TLCP `ECC_SM4_GCM_SM3`（-gm 形态） | 2.4 ms/流，117 KB / 538 allocs 每流 | SM2 双证书握手比 RSA-2048 形态便宜 |
| 单流吞吐 · 经网关 TLS 1.3 `AES_128_GCM` | **1.1 GB/s** | 直连 TCP 对照 6.1 GB/s：网关（TLS 终止 + 用户态转发）在单流上约 5.5× 于裸 TCP 的成本；真实链路上限通常在网络而不在这里 |
| 单流吞吐 · 经网关国密 TLCP `SM4_GCM` | **166 MB/s** | 比 TLS 1.3 慢 **6.6×**——gmsm 的 SM4-GCM 是软件实现，无 AES-NI 同款硬件加速；国密隧道的单流带宽上限在这里 |
| 并发建流 · limit=256、16 条并发（RSA） | 2.15k 建流/s，0 拒绝 | 8 核进程内、客户端与网关同抢 CPU，**不是**单机 conn/s 规格 |
