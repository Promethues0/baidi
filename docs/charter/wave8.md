...
> **wave8 已全部落地（2026-08-18）**：17 条行动逐条完成、提交、演示站部署并实机复验。
> 累计约 130 条变异逐条转红，5 次「绿着的测试在替坏行为背书」被改正。
> 本波新沉淀的两条判据已写进 CLAUDE.md 与 docs/ARCHITECTURE.md 第七节：
> **入口比实现宽**（校验层放行执行层必然拒绝的值，而拒绝发生在很久以后、很远的地方）与
> **默认值就是绝大多数部署的真实姿态**（"生产建议开"的开关默认关就等于没有）。
...
**13. 四块假配置面一次收口（α 组）— M ✅ 已落地**
- 落地记（2026-08-18）：四块逐一二选一，没有一块维持现状。
  - **①摘除 + 换真**：「用户策略 · 继承编辑器」整套（8 项 × 继承树 × 打破继承 × 30s 撤销条 × 影响预览）连同 `policy_overrides` 读写、`OrgNode.HasCustom`、`GET/PUT /policies/{node}`、`GET /policies` 一并摘掉（表保留，一张没人读的空表不构成风险）。换上 FR-POLICY-29/30 两条 P0，执行点唯一：`api.accessSessionGate` → `handleKnockToken`（第五道闸，排在设备准入之后）。
  - **②比原方案更彻底**：主认证下拉不是"没接线"而是**同义反复**（`Match` 第一行就按目录筛），所以删而不是置灰；顺着查出同抽屉两处同族问题——**PC/移动端两栏合并**（`loginCtx` 里没有端标识，三端同一个 `/portal/login`；落库两列保留，读时并集、写时 pc 存全量 / mobile 存空，无损且省掉一次会漏跑的迁移），以及 **`Secondary` 此前有能力声明却零消费方**，现在给了唯一执行语义：列了方式的策略**不接受 legacy 演示验证码回落**（裸 IP 演示站正是它生效的地方；留空 = 行为逐字不变）。
  - **③删除** `AuthzApps`（零执行方，还把空值渲染成「不授权」）。
  - **④`type` 删除、`scope` 接真**：`scopeOrgs`/`scopeGroups` 在 `risk.Evaluate` **之前**过滤（`api.baselinesInScope`——Evaluate 是纯函数，塞进取数就再也测不动了），组织含子树，判定收敛到 `store.SubjectIndex.Covers` 一处（授权/策略/基线三处共用，`authpolicy.covers` 同批改成调它）。两栏都空 = 对全体生效（与自由文本时代逐字一致，回填后判定不变）；展开索引读不到时**保留全部基线**（基线是闸门，读失败不该让全体瞬间"合规"——与「认证源不可用绝不动手」方向相反是有意的）。
- 设计上最要紧的四条（都写进代码注释了）：**①`MaxDevices==0` 不能兼任"不限"**（PRD 明写 0=禁止登录，存量库零值 = 升级重启那刻全员被挡在门外，故另设 `DeviceLimitEnabled`；`ParseAccessPolicy` 对坏数据一律回落成"两条都关"）；**②判据是业务流量不是敲门保活**（客户端不退出就每 15s 敲一次，拿保活当活跃 = 规则永不触发，又一条页面显示"已启用"而从不生效的安全规则）；**③`lastActive` 必须是三态指针**（nil=旧网关不报 / `&0`=从未有业务连接 / `&ts`；压成 `int64+omitempty` 会让一台没升级的网关每 15s 给所有会话盖 1970 年的戳，一开规则全员被踢）；**④名额先到先得而不是"已在线就放行"**（后者会让上限从 5 调到 2 之后新上限永远不生效），且**新终端永远排在已在线的后面**、**被拒的新终端要回滚记账**（记账在判定之前，留着会在下一轮排名里挤掉一台真在线的）。
- 变异验证 13 条全红；其中 3 条第一次是逃逸的，每条都是构造的输入区分不了两种实现：`existed` 恒真（要一个指纹字典序靠前的新终端才验得到）、探不到当没流量（用例误传 `existed=false`，整条 idle 分支根本没执行）、旧网关 nil 当 0 落库（没有任何用例走心跳→活跃回执这条路，补了三态回执用例）。另有一条 `Touch` 的窗口外守卫**确认无法用行为区分**（Sessions()/Allowed() 都按 until 过滤），如实标注为条目泄漏防护而不是硬claim。
- 本机真栈实测五种形态（默认不生效 / 上限调小真把最晚接入的挤掉 / 先到的续期不掉线 / 无业务流量注销后关规则也不自愈 / 重新登录恢复且不立刻再被踢），控制台三页浏览器实测（含「展开后 0 个账号，这条基线当前对谁都不生效」）；演示站部署后复验接入闸真拒真放、旧端点 404、既有 pc/mobile 行合并无损。
- 顺带修 `SCOPE.md ch12`（那句「砍安全基线管理」与代码相反）与 ch10（从一句规划名改成逐条实况）。
...
**14. 应用可改可删 + WEB 全网资源模式卡处置（FR-APP-01/02/03、FR-NET-01/02/03/05）— M ✅ 已落地**
- 落地记（2026-08-18）：①`PUT/DELETE /api/v1/apps/{id}` 补齐，控制台「编辑」进真编辑抽屉、旁边那个没有 `@click` 的「详情」删掉、新增「下架」并带确认弹窗。②「WEB 全网资源」**正名为「直连书签」**（三处名字统一）+ 向导与编辑抽屉各挂告警 + 门户打开行为改对。顺带摘掉 `apps.node`。
- 四条纪律都写进了代码注释：**路径 id 说了算、请求体 id 一律忽略**（否则「编辑 A」改到 B 身上而 URL 与审计都记 A）；**UPDATE 必须查 RowsAffected**（SQLite 对不存在的 id 不报错 → 改一条已被别人删掉的应用静默成功，刷新后改动凭空消失）；**下架不级联删受控资源**且回执与弹窗都说清「访问控制仍按资源策略生效」（不说的话管理员以为下架顺手收权了，而 ACL 与 JIT 授予原样有效、隧道照样连）；**重复下架回 404 不回 200**（回 200 会落一条「下架了」的审计，而库里根本没有这一行）。
- ②的处置是**正名而不是删功能**：一个门户书签本身有用，坏的是它伪装成受控发布形态。顺带修掉一处**指错路**的提示——此前门户对 global 统一说「浏览器无法直达，请用桌面客户端接入后访问 X」，而它压根不走隧道，客户端接入了也帮不上忙。现在完整 URL 直接开新标签、泛域名如实说明，按钮文案（打开链接 / 查看地址 / 访问 / 接入地址）与真实行为一一对应。
- **差点踩进去的坑**：`app_categories` 里也有一个 key=`global`（标签「全网资源」），那是**分类**维度，与发布形态的 `global` 只是同名。第一版顺手把分类标签也改成了「直连书签」——正是 CLAUDE.md 里那句「两者恰好都有一个叫 global 的取值」警告过的混淆，已撤回并在种子里留注释挡住下一次。
- 变异验证 4 条全红：UPDATE 不查影响行数 / 请求体 id 优先 / 下架级联删资源 / `node` 字段回归下发。本机真栈实测编辑原地生效（总数不变、旧名字消失、审计写出改前改后）、下架后资源仍在、重复下架 404；控制台与门户浏览器实测（表格「关联资源」列、编辑抽屉回填与 global 告警、门户「打开链接」按钮）。
...
**15. 灰度链收尾（γ 组：FR-UPG-19 AC-12 + FR-OPS-08/09）— S ✅ 已落地**
- 落地记（2026-08-18）：三处全接上。①灰度弹窗加真的用户组多选、`openGray` 回填 `p.groups`、保存原样带回——此前请求体写死 `groups: []` 而 `SaveGrayPlan` 是整条覆盖式保存，管理员只把比例从 10% 调到 20% 就会把定向清空（回 200、页面看不出差别）。②`upgrade.Coverage` 接进 `/upgrade`（`coverage`/`total`/`groups` 三个字段），表格新增「预计影响 N / M 人」列。③移动端「我的」页进入即查 `GET /client/update` 并出横幅，手写的 `v0.1.0` 换成构建期从 `package.json` 注入。另加**现场终端实际版本分布**（`ClientVersionStats`）——那是 AC-12 在真机上唯一的验收依据。
- 三条判据写进了注释：**分母是目录全部账号**（只数在线的会让同一条计划在不同时刻显示不同覆盖数，而分桶确定性，那种跳动会被读成"灰度不稳定"）；**读取失败字段缺席、页面显示「—」不是 0**（画成 0 人，管理员的下一步动作是调高比例）；**「未上报版本」单列一桶**（并进具体版本会让"有一批机器没报过版本"这件事消失，而那批机器恰恰最该盯）。
- **顺手修掉一处自造的措辞矛盾**：弹窗预览第一版写的是「分桶是确定性的，这个数字是精确算出来的」，而前端算不出服务端的 SHA-256 分桶、比例那半本来就是估算——写成"精确"就是替一个估算值背书。改成「定向命中 N 人（精确）＋ 按 X% 分桶约 M 人（估算）」，并指明保存后表格那一列才是后端精确数出来的。
- 变异验证 3 条全红：Coverage 不接 SubjectIndex（按用户组定向永远 0 人命中）/ 覆盖数改回比例估算 / 版本分布丢掉「未上报」桶。本机真栈实测：建 2 人用户组 + percent=0 → 覆盖数 2；percent=20 / 8 人 → 覆盖 2；posture 造出 macOS 0.4.0 ×2 与 Windows 未上报 ×2，分布逐条对上；控制台实测弹窗多选、回填、「2 / 8 人」列与版本分布表。
- 顺带按「注意」改掉 `SCOPE.md:11` 那句——**一个会吞配置的页面不属于"已知限制"，那是缺陷**。
...
- **演示站实测**：40 次快速登录 → 11 通过（burst 10 + 速率 1）、29 个 429，其中前 5 次错误口令先触发了 IP 维度锁定（403）——两层各自按设计生效，且**把自己锁在了外面 15 分钟**。顺带发现一条运维提示并写进了 ARCHITECTURE：绕过端点直接 `DELETE FROM login_lockouts` 只清库不清进程内那份，要重启 control 才生效；正规解锁走 `Guard.Unlock`（用户状态页那个按钮）两边一起清。控制台正常并发（20 个并行请求）全 200，不受 `/api/` 那道 30r/s 影响。
...
> **第 1、2、3 条已落地（2026-08-18）**：`SCOPE.md` ch21 那个盖住整章的 ✅ 已拆成六小节逐节表态
> （21.1 认证源真 / 21.2 日志外送真 / 21.3 SSO 代填不做 / 21.4 数据模型无独立交付 /
> 21.5 开放平台 API 不做且缺的是三件 / 21.6 SDK 只有移动端那半），ch13 那句空引用同批改掉，
> `ARCHITECTURE.md` 第七节新增一张表逐条写明 SSO 代填 / 开放平台 API / 第三方风险摄入 /
> URL 级发布（含 webproxy 边界此前唯独没提的路径粒度）。第 4~8 条留待 wave9。
...
7. **Windows / Linux 桌面端真机验证债（不是代码债）。** ——**Windows 阶段 A 已还（2026-08-18）**：
   一台 Windows 11 ARM64 真机跑 `verify-windows.ps1 -Stage A`，7 项全 PASS（安装目录
   `C:\Program Files\白帝安全接入客户端\`、perMachine 落位、wintun.dll 与 baidi-tun.exe
   同目录且均 arm64、许可随包、sidecar 命名一致）。这是这条链路**第一次有真机证据**，
   证明的是那一整串打包纪律真落成了声称的样子；**UAC 提权 / 建卡 / 路由 / NRPT DNS /
   隧道连通一条都没覆盖**，UNVERIFIED 与"不进下载中心"此刻仍然成立。阶段 B、C 待跑。
   详见 clients/BUILD.md 10.3b。 `clients/BUILD.md:533` 已如实写明「Windows 这条链路至今从未在真实 Windows 上跑过」，包能出、wintun 随包、哈希钉扎、NSIS perMachine、三道守卫都做了，**缺的只是一台真机**。列在这里是为了别让它继续隐形——不建议在 wave8 里当代码行动做，没有真机时任何「修复」都不可证。
