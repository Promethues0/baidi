...
**8. last_login 写入方 + 闲置账号治理（FR-MON-19/20）— S+M ✅ 全部已落地**
- ②落地记（2026-08-14）：`store.ClassifyIdle` 纯函数三态判定（可解析 last_login → 真闲置天数；占位符/历史行 → created_at 兜底并标「无登录记录」；都解析不出 → 不可判定不列入——判不了不当闲置）。`GET /users/idle?days=N`（读=任意管理员，阈值夹 [7,3650] 默认 90）+ `POST /users/idle/lock`（PermSecurity；管理员目标逐个抬 PermAdmins、最后一名超管由 store 防自锁拦、逐账号数据面撤窗断隧道+落审计——批量只是循环，不是绕守卫的通道）。Users.vue 工具栏「闲置治理」弹窗，管理员单独标记且默认不勾选。浏览器实测：li.fang 因当天真实登录不在清单（8① 写入方的活证据）。
...
**13. 图形验证码：接真或摘除（FR-SCEN-13）— S ✅ 已落地（判定：摘除）**
- 落地记（2026-08-15）：**摘除**，理由写进 ARCHITECTURE 第七节「图形验证码：明示不做」。判定过程用了一次对抗验证（一个 agent 专职反驳"摘除"、一个评估"接真"成本）：结论 stands，但**证伪了两条支撑论证**，并挖出一条带 PoC 的真实缺陷（见下）。接真成本评估：700–900 行 Go + 240 行前端 / 15–17 文件，且生成器源码在仓库里 = 攻击者拥有无限量完美标注训练集。
- 摘的不止一行：Policy.vue「全局策略」六个开关全是纯前端摆设（保存不提交、后端零消费方），整批换成如实的「本版本未实现 + 为什么」。其中「禁止用户通过浏览器登录」比验证码危险得多——它看起来能关掉七层 Web 代理这条免客户端接入路径。
- **附带的安全加固（本行动真正的价值）**：对抗验证的 PoC 证明防爆破可被廉价绕过——失败计数表按「最后一次失败最早」淘汰，攻击者在两次猜测之间灌一批新键就能把受害账号的计数挤掉，连猜 50 次零锁定且全程静默。已修：两维度分表各自限额、淘汰改「失败次数最少优先」、IPv6 按 /64 聚合、锁定表加上限、表满落 warn 不再静默。四条回归用例 + 一条 `TestKnownLimit_` 如实钉住修不掉的那半（有界表 + 无限键在应用层无解，需前置 nginx 限速）。顺带补上 `POST /auth/password` 旧口令校验漏接的防爆破闸（持令牌者的口令预言机）。
