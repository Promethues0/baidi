...
- **入口主机名读管理员登记的网关接入地址，推导出回环即不就绪**（`api.webEntryBase`）。取数优先级：
  ...
  `gatewayAccessMap`、同一张 `gateway_access` 表，内网栏优先与剖面「内网在前」同序）→ 网关自报的七层监听
...
- **登记地址 + 自报端口只对「网关 L7 口直接对外」成立**。走第 3 档时 scheme 取自网关自报的 `webTls`、
  端口取自它的 `-web` 监听（与剖面「端口的权威来源是自报监听地址」同一条纪律）；经前置 nginx 终结 HTTPS
  的部署对外是 443 而网关监听 18444，登记地址算出的 `http://gw:18444` 仍然打不开——那种拓扑要配
  `BAIDI_WEB_ENTRY_BASE`（或资源级 `webEntry`）指向 nginx。控制面无从区分这两种拓扑，只能把规则写在这里。
