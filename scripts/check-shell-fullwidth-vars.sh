...
#   精确复现：
#     LC_ALL=en_US.UTF-8 bash -c 'set -u; A=1; echo "（$A）"'  → A�: unbound variable
#     LC_ALL=C           bash -c 'set -u; A=1; echo "（$A）"'  → （1）
#   本项目所有提示语都是中文，这个写法极易复发。
#
# ★`set -u` 让它当场炸是运气好：没开 `set -u` 的脚本里它**静默展开成空串**——
#   `rm -rf "$PREFIX/x"` 这类地方错成什么样不用多说。真实踩到过的一次是
#   deploy/build.sh 的限流自检：报错句写成「缺少限流指令「$d」」，在 mac 上恰好
#   丢掉它唯一要传达的信息（到底是哪一条没了）。
#
