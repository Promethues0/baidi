...
func (s *Server) handleEnter(w http.ResponseWriter, r *http.Request) {
	...
	c, err := VerifyTicket(s.cfg.Verifier, r.URL.Query().Get("t"), s.cfg.TicketMaxTTL)
	if err != nil {
...
func (s *Server) handleAny(w http.ResponseWriter, r *http.Request) {
	...
	rr := r.Clone(context.WithValue(r.Context(), ctxKey{}, routed{res: res, user: sess.User, prefix: AppPrefix(resID)}))
	rr.URL.Path = rest
...
func (s *Server) rewrite(pr *httputil.ProxyRequest) {
	...
	// ★先剥后写，顺序不能反：剥的是浏览器自称的来源，写的是 net.Conn 的真实对端。
	StripInboundHops(pr.Out.Header)
	proto := "http"
	if s.cfg.TLSTerminated {
		proto = "https"
	}
	SetForwarded(pr.Out.Header, hostOf(pr.In.RemoteAddr), pr.In.Host, proto, rt.user, rt.res.ID)
}
