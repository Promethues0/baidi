...
| **服务端分页** | NFR-PERF-06（P1） | ⏸ **记档延后**（wave8 建议 5 已记，此处补齐编号）。用户目录 / 资源 / 应用三个清单端点全表返回；`GET /client/update` 的 `groupsOf` 为取一个账号的组 id 会全量拉起整张用户目录。失败形态是**变慢（可见）**，不属静默失效族，故排在静默缺陷之后 |
