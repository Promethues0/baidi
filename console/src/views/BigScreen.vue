<template>
  <div class="scr">
    ...
    <header class="scr-top">
      ...
      <h1 class="scr-title">零信任安全态势感知中心</h1>
    ...
    <div class="scr-grid">
      ...
      <section class="scr-col">
        <div class="panel">
          ...
          <div class="kpis">
            <div class="kpi">
              <div class="kpi__v">{{ ov.devices.online }}<small>/{{ ov.devices.total }}</small></div>
              <div class="kpi__l">在线设备 · {{ (ov.devices.rate * 100).toFixed(0) }}%</div>
            </div>
            <div class="kpi">
              <div class="kpi__v">{{ ov.sessions }}</div>
              <div class="kpi__l">活跃会话</div>
            </div>
            <div class="kpi">
              <div class="kpi__v">{{ ov.users.total }}</div>
              <div class="kpi__l">纳管用户 · 禁{{ ov.users.disabled }}锁{{ ov.users.locked }}</div>
            </div>
            <div class="kpi kpi--danger">
              <div class="kpi__v">{{ threatTotal }}</div>
        ...
        <div class="panel">
          ...
          <div class="donut">
            ...
            <div class="donut__c"><b>{{ verdictTotal }}</b><i>今日判定</i></div>
      ...
      <section class="scr-col scr-col--c">
            ...
            <svg viewBox="0 0 400 400" class="radar-svg">
              ...
              <g v-for="(p, i) in blips" :key="i">
                ...
                <circle :cx="p.x" :cy="p.y" :r="p.r" :stroke="p.color" class="blip-ring">
                  <animate attributeName="r" :values="`${p.r};${p.r + 14}`" :dur="`${p.dur}s`" repeatCount="indefinite" />
                  <animate attributeName="opacity" values=".7;0" :dur="`${p.dur}s`" repeatCount="indefinite" />
                </circle>
              </g>
            </svg>
        ...
        <div class="panel panel--lines">
          ...
          <div class="gauges">
            <div v-for="d in ov.defense" :key="d.key" class="gauge">
              ...
              <div class="gauge__c">
                <b :style="{ color: riskHex(d.risk) }">{{ d.risk }}</b>
...
/* 三道防线仪表（270° 弧） */
const gaugeR = 52;
const gaugeLen = (gaugeR * Math.PI * 270) / 180; // 弧长
...
const blips = computed<Blip[]>(() => {
  ...
  return uniq.map((it, i) => {
    const ang = (i * 137.508 + 18) * (Math.PI / 180);            // 黄金角散布
    const rad = 50 + ((i * 53) % 120);                            // 半径 50~170
...
.scr {
  --c-bg0: #060c1f; --c-bg1: #0a1838; --c-cyan: #2fe6ff; --c-blue: #4080ff;
  --c-line: rgba(96, 150, 255, .16); --c-panel: rgba(20, 44, 96, .35);
  --c-t1: #e6f0ff; --c-t2: #9db4e6; --c-t3: #6a82b8;
  position: fixed; inset: 0; height: 100vh; width: 100vw; overflow: hidden;
  background:
    radial-gradient(1100px 600px at 50% -10%, rgba(47, 230, 255, .10), transparent 60%),
    radial-gradient(900px 600px at 100% 100%, rgba(64, 128, 255, .10), transparent 60%),
    linear-gradient(160deg, var(--c-bg1), var(--c-bg0));
...
.scr-title {
  ...
  background: linear-gradient(90deg, #9fe9ff, #fff 45%, #9fc6ff); -webkit-background-clip: text; background-clip: text;
  color: transparent; text-shadow: 0 0 24px rgba(47, 230, 255, .25);
}
...
.scr-act:hover { color: var(--c-cyan); border-color: rgba(47, 230, 255, .5); }
...
.panel::before, .panel::after { content: ''; position: absolute; width: 14px; height: 14px; border-color: rgba(47, 230, 255, .55); }
...
.panel--grow { flex: 1; }
.panel__h { display: flex; align-items: center; gap: 9px; font-size: 14px; font-weight: 700; color: var(--c-t1); margin-bottom: 12px; }
...
.kpi__v { font-size: 30px; font-weight: 800; line-height: 1.1; color: #fff; text-shadow: 0 0 16px rgba(47, 230, 255, .35); }
...
.kpi--danger .kpi__v { color: #ff6b6b; text-shadow: 0 0 16px rgba(255, 77, 79, .4); }

/* 判定环图 */
.donut { display: grid; grid-template-columns: 130px 1fr; align-items: center; gap: 6px; position: relative; }
.donut__svg { width: 130px; height: 130px; transform: rotate(-90deg); }
.donut__track { fill: none; stroke: rgba(96, 150, 255, .12); stroke-width: 11; }
.donut__seg { fill: none; stroke-width: 11; stroke-linecap: butt; transition: stroke-dasharray .5s; }
...
.bar__fill { display: block; height: 100%; border-radius: 5px; background: linear-gradient(90deg, var(--c-blue), var(--c-cyan)); transition: width .5s; }
...
.radar-sweep {
  border-radius: 50%;
  background: conic-gradient(from 0deg, rgba(47, 230, 255, .42), rgba(47, 230, 255, .05) 55deg, transparent 120deg, transparent 360deg);
  animation: sweep 4.5s linear infinite;
  -webkit-mask: radial-gradient(circle, #000 0 89%, transparent 90%);
  mask: radial-gradient(circle, #000 0 89%, transparent 90%);
}
...
.blip { filter: drop-shadow(0 0 5px currentColor); }
...
.gauge__val { fill: none; stroke-width: 9; stroke-linecap: round; transition: stroke-dashoffset .6s; filter: drop-shadow(0 0 4px currentColor); }
...
.rg__rk.hot { background: linear-gradient(135deg, var(--c-cyan), var(--c-blue)); color: #06122e; }
...
.rg__fill { display: block; height: 100%; border-radius: 5px; background: linear-gradient(90deg, var(--c-blue), var(--c-cyan)); transition: width .5s; }
