...
| macOS（缺 dmg 时） | *（空着，什么都不说）* → 一度改成「构建中，敬请期待」 | 本部署尚未放入 macOS 安装包（dmg 由 CI 或本机 clients/build-artifacts.sh 产出后铺进下载目录，控制面不会自己构建）；请联系管理员 |
| Android（缺 APK 时） | **构建中，敬请期待** | 本部署尚未放入 Android 安装包（调试签名 APK 由 CI 产出后铺进下载目录，控制面不会自己构建）；请联系管理员 |
...
**macOS 与 Android 后来也不再写「构建中」**（2026-09-07）。此前的判据是"真的会被构建出来、且装了能用"，
这两个平台确实满足；但那四个字说的不是"能构建"，是"**正在**构建"——一句状态断言，而控制面上没有
任何执行方能证明它：manifest 缺失只说明这台部署机上没人跑过 `build-artifacts.sh`，此刻什么都没在构建。
本机起栈、从没铺过包的开发环境，下载中心就对这两行说了几个月的「构建中」。现在六个平台的占位说的是
同一件事——包不在这台机器上、谁能把它铺进来、找谁；守卫也从四个平台的名单改成遍历
`placeholderManifest()` 的全部条目，将来加第七个平台自动进名单。
...
**验证阶段 C 的前提因此要写清楚**——而且这里是**两条独立链路**，别只补一条：

| 链路 | 走谁的信任库 | 能怎么补 |
|---|---|---|
| 登录 / `api.ts` 的每一次 fetch（WebView2 · WKWebView） | **操作系统**信任库 | **只有**「把控制面证书导进本机信任库」或「给控制面换一张受信证书」两条；客户端侧**没有入口可加**——Tauri 的 webview 没有安卓 NSC 那样的按域信任配置 |
| `baidi-tun` 取敲门令牌（Go 的 `crypto/x509`） | Go 的信任池 | 上面两条同样管用；另外可以单独喂锚：`baidi-tun -control-ca <PEM 路径>`（不传即用系统信任库，**不是**跳过校验；信任池是系统池 ∪ 锚）。**桌面客户端里这个参数不用手填**——把 PEM 放到约定路径 `<家目录>/.baidi/control-ca.pem`（Windows 是 `%USERPROFILE%\.baidi\control-ca.pem`），客户端拉起 sidecar 时自己带上；★权限要对（**0600、属主是你本人或 root、不能是符号链接，父目录同判**），不对时**当场拒绝接入**并说明原因，不会悄悄回落 |

**只补数据面那一条是不够的**：登录都过不去，就永远走不到拉起 `baidi-tun` 那一步（本节开头那条因果链）。
反过来只导证书是够的——所以自建自签部署下的标准姿态**仍然是导证书**；`-control-ca` 是给
「不便动本机信任库」的场合留的第二条路，也让数据面那半边的失败能被**单独**排掉，
而不是永远和登录失败混在一起（10.3e → 10.3f 那条排查线最费时间的一段正是这个）。

★**演示站现在不用导了，但别把这条记成「已经不需要了」**：101.43.125.131 自 2026-09-08 起挂的是
Let's Encrypt 的 **IP 地址证书**，**有效期只有 6 天 15 小时**，续不上时续期脚本会把 nginx 指回自签
（缘由与运维见 deploy/README.md「HTTPS 证书」）。所以跑阶段 C 之前**先确认它此刻还受不受信**：

```bash
curl -sS -o /dev/null -w '%{http_code}\n' https://101.43.125.131/    # 注意：不带 -k
```

回 200 才算受信；回非 200（TLS 失败）就是已经退回自签姿态，照旧要先导证书。
...
1. ~~**取敲门令牌失败（演示站证书）**~~ —— **已修复，端到端跑通**。
   ...
   **2026-09-03 真机端到端验收（OPPO PKU110 / Android 16 / 演示站当时仍是自签、一个字节没动）**：
   ★**这个前提在 2026-09-08 变了**——演示站换成了 Let's Encrypt 的 IP 地址证书（见 deploy/README.md
   「HTTPS 证书」），于是**对着演示站再跑一遍已经证明不了「锚生效」**：不带 `-PbaidiControlCa` 的包
   现在也能登进去、也能取到令牌（走的是系统信任库）。**连下面那两道反例对照一起失效了**（两道都
   依赖「系统信任库不认这张证书」这个前提）。要复验这条链路，只能换一台自签部署去验——
   别为了做实验去把演示站的证书改回来。
