...
| `Overview` / 监控中心 + 大屏 | `Devices`（186/240）、`Sessions`（186）、三道防线的风险分与 TOP 实体（`203.0.113.7`、`svc-bot-04`、`WIN-诊室-12`）——与同屏真实的用户统计、审计聚合并排 | 设备改 `trusted_devices` **台账口径**（登记 / 已授信 / 待审批 / 已吊销 + 纳管率；"设备此刻在线"控制面无从得知，会话按账号计、没有设备维度）；`Sessions` 在 store 层恒 0、由 api 按网关上报注入；三道防线分别取自 `attack_sources`、`users`+posture、posture。`DefenseLine.Trend` 字段**删除**（趋势要历史快照，白帝一张历史态势表都没有）。**wave11 行动 12 补齐后半程**：账号防线的风险账号此前仍读 `users.risk`（全仓只有 INSERT、无一处 UPDATE 的死列，种子里 `li.fang`/`ext.zhou` 恒 high），现改为经 `store.RiskOfDisposal` 按 posture 最新判定现算（与「用户与角色」页、在线用户页同一处折算表）；终端防线此前返回的是**账号名**、与账号防线逐字相同，现按 `(账号,设备指纹)` 逐台聚合（`平台 · 指纹短码 · 判定档 · 账号`）。`DefenseLine.Risk` 同批改成**三态指针**（nil = 一份判定材料都没有），另加 `Unknown` 计数 |
...
- **内核 IP 转发白帝只读不写**（wave11 行动 10-①）。分支主机的流量要进隧道，必须先被内核从局域网口**转发**到 `baidi-ipsec` 的 TUN 上；转发关着时 IKE 全绿、SA 倒计时正常、站点显示「已建立」，而 rx/tx 恒为 0——包在内核路由那层就没了，ESP 引擎从没见过它，连 `dropHint` 那七个丢弃计数都是 0，**整条链路上没有任何一处会报错**。现在 `internal/kernelfwd.Probe()` 按站点地址族实测该开关并随状态回报，控制面 `api.ipsecForwardReceipt` 折成五态（`on`/`off`/`unknown`/`n-a`/`unreported`）逐站点呈现。**它是回执不是判定**：不改写站点五态（转发关着的隧道确实建起来了），也**不替管理员改 sysctl**——那是宿主机的全局网络行为。探不到一律 `unknown`（塌成 `off` 是虚警、塌成 `on` 是替一台可能什么都不通的机器背书）；`n-a` 专指 `-datapath=netstack` 自检数据面；`unreported`（旧网关没报过）与 `unknown`（报了但探不到）分开呈现，两者的下一步动作不同。**只覆盖 Linux/macOS**，别的平台如实报不可判定。
- **同一台承载网关上不允许两条对端 IP 相同的站点**（wave11 行动 10-②）。响应方 `ike.findSiteByPeer` 在 IKE_SA_INIT 阶段**只有对端 IP 可用**（IDr 要到 IKE_AUTH 才出现，TS 更在其后），协议层面区分不开。入口 `api.ipsecPeerConflict` 保存即拒并说明原因（照 `peer` 拒收 FQDN 的先例）；判据是 **(gatewayId, 对端 IP)**——少了 gatewayId 会误伤「两台组网网关连同一对端」这种合法拓扑（`ipsec-e2e.sh` 正是这个形状），而端口不参与（`findSiteByPeer` 刻意不比端口，按带端口的串去重的话改个端口就能绕过）。数据面侧从 map 迭代序（**随机**）改成按站点 id 字典序**确定性**选择 + 5 分钟节流的告警；存量库里已有的重复由读端 `ipsecDuplicatePeers` 在**两条**站点上都挂告警并点名「谁会被选中」。改造前的现场是：对端每发起一次协商就掷一次骰子，命中错的那次会把**一条本来健康的**站点打成 failed，而 `LastError` 写的是「对端的提案不可接受」——方向完全相反，且下一次可能又好了。
...
- **温备进入了版本一致性视野**（wave11 行动 18-③）。改造前**控制面根本不知道备机上那份 `baidi-control` 是哪一版**，切换脚本也不比对，而升级页的边界文案还断言「温备节点上没有需要升级的服务端进程——它只跑 baidi-standby」——那是**错话**，而且方向危险：备机上跑着 `baidi-standby`（本身就是要跟着换版本的服务端进程），还装着 `baidi-control`（提升脚本最后一步 `systemctl start baidi-control` 启动的就是它，也是切换后真正跑起来的那一份）。照那句话理解，运维会只升主机，而切换那天备机上是旧版控制面去打开一个被新版迁移过的库。
  现在备机每轮同步回报**两组**版本身份：自身（`baidi-standby`）与它**实测**到的同机 `baidi-control`（跑一次 `baidi-control -version`，输出是一行 JSON）。判据只用后者——两个二进制同版是部署脚本的约定、不是可核实的事实，而"上一次部署只覆盖了其中一个"恰恰是最该在切换前发现的形态。主机侧折成三态：`match` / `mismatch`（**翻 warn**，切换那天真会出事）/ `unknown`（**不翻状态**，只如实呈现——旧备机不上报是全部存量部署的形态，把它当告警会让每套部署在升级当天集体变黄；但它也绝不能显示成"一致"，`"" == ""` 走进 match 分支是这一处最容易写坏的地方）。四个新列**刻意不回填**：空串在判定里恰好就是"不可判定"，回填成主机版本 = 替一台还没说过话的备机宣布"我们一致"。
- **切换脚本给出终端侧接管的校验与清单**（wave11 行动 18-④，**刻意不做自动接管**）。改造前脚本末尾只交代「各网关的 `-control` 指向这台机器」，而桌面端的控制中心地址是本机 `localStorage` 里一个**手填的单值**（无故障转移列表）、移动端的地址与控制面信任锚都**编译进安装包**——切换完成那一刻，这两类终端全都还指着旧主机，且不会自己找过来。**自动接管做不了也不该假装做**：它需要一个终端能主动发现新主机的服务（DNS SRV / 引导端点 / 锚点服务），白帝一个都没有。所以脚本干跑时做三件事：① 查本机 `baidi-control` 在不在、是哪一版（不在 = 提升最后一步会失败）；② 从 `latest.json` 的 `primary` 读出旧主机地址，并核对**本机是否已持有那个地址**（VIP 漂移 / 同地址接管 → 终端侧一动不用动；查不出来时按"要改"提示，方向是故意的）；③ 逐类列出必须人工做的动作。另有一条只在这里说得清：**HTTPS 证书不在备份材料清单里**（`backupSources` 只含库 + 内部 CA + 四把签名密钥 + 审计链密钥），每台机器的自签证书是 `install-remote.sh` 各自生成的，于是切换后这台出示的是**另一张叶子**，而桌面端 `~/.baidi/control-ca.pem` 与安卓包里编译进去的锚钉的都是旧主机那张。要让切换不牵动终端，唯一的正路是两台共用一个对外入口（同 VIP / 同域名 + 同证书），不是切换当天补救。这一屏文案由 `TestPromoteListsTerminalSideTakeover` 钉住——它本身就是交付物。
...
- **切换后终端侧不会自动接管**（见上一节末条）。白帝没有让终端发现新主机的服务，脚本给的是校验与清单，动作仍是人工的；**移动端只能重新出包**（地址与信任锚都编译在包里）。
- **控制面不知道备机上有没有装 `baidi-control`**，除非备机自己报上来（`baidi-standby` 每轮探同目录那一个）。探不到时如实报"不可判定"，不推断成"没装"也不推断成"和主机同版"。
...
| 增强 · 弱密码 | `users.pw_strength = weak` | 本地账号：**改密/建号那一刻**按明文判定（`auth.PasswordStrength`）——登录时只有 bcrypt 哈希，判不了。外部口令认证源（LDAP/AD/RADIUS）账号：**每次认证成功那一刻**按同一套判据判定（`api.noteExternalPwStrength` → `store.SetExternalPwStrength`，只落强度标记）。OIDC 拿不到口令，恒 `unknown` |
...
- **加严配置有防自锁闸**（FR-ADMIN-20，wave11 行动 6）：一条 `enhance.always` + `secondary:["totp"]` 的本地默认策略，保存下去就把全部管理员永久挡在管理台之外——管理台与门户走同一个 `secondFactor`，而注册第二因子的入口（`/totp/enroll`、webauthn 注册）都要求**先登录**。闸挂在写之前（`api.guardAuthPolicyLockout`，保存与删除两个端点都过，409 + 原因 + `deny` 审计），判据是「改完之后是否还存在至少一名**能登进管理台、且改得动这条策略**的管理员」：账号未被禁用/锁定 ∧ 有本地口令（管理台只验本地 bcrypt）∧ 持 `PermSecurity`（登得进来 ≠ 救得了场）∧ 过得了二次认证。求值走 `authpolicy.EvaluateLockout`——**豁免不算退路**（可信网络/授信终端取决于他届时从哪登，保存那刻无从知道）、**非工作时段不算锁死**（可等待；用 `time.Now()` 求值会让同一条策略白天存得下、晚上存不下）、**RP 未配置时 passkey 不算第二因子**（`secondFactor` 第一步整个包在 `if s.webauthnEnabled()` 里）。「进不进得去」的判据本体是 `authpolicy.Blocked`，**登录链路与闸调同一个函数**——各写一份的话闸放行而登录挡住，两处都不报错。**闸是差分不是绝对**：改动前就已锁死时不拦，否则会把唯一能修好它的那次编辑一起拦掉。
- **被挡住时给的补救路径真实存在**（同上一条的另一半）：原文案里的「可联系管理员协助录入」与「请先在门户「安全设置」里绑定后再登录」指的都是**不存在的路**——白帝没有任何"管理员代为录入认证器"的入口（管理员侧只有 `ResetWebauthnCredentials`，那是清空），而门户「安全设置」本身也在同一堵墙后面（同一条策略按目录匹配，两处都调 `secondFactor`）。现由 `api.enrollDeadEndNote` 一处给出两条真出路：命中豁免的网络/终端登录后立即注册，或请另一名还进得来、且持「安全策略」权的管理员改这条策略——后者之所以成立，正是因为有上面那道闸顶着。

**不能声称 / 刻意不做**：

- **防自锁闸只覆盖认证策略自己的写端点**：把管理员移进某个组织 / 用户组、或改一个被策略引用的组的成员，同样可能让他落进加严范围，那几条路**未接闸**。闸也**不是事务性的**（判定与写之间有一个窗口，并发的两次保存理论上仍可能各自以为"对方那边还有人"）——收在事务里要跨 `users`/`admin_roles`/`totp_secrets`/`webauthn_credentials`/组织展开五处取数，代价与收益不成比例；同族的「最后一名超管」那道闸是事务内计数，因为它只数一张表。
...
- **外部目录账号的强度标记在「外部认证成功那一刻」落（wave11 行动 8-③，FR-AUTH-21）**。改造前这一列对外部账号**恒为 `unknown`**：`BindExternalUser` 写 `pass_hash=''`，两个写值方（自助改密 / 建号重置）外部账号一条都走不到，于是「弱密码」增强规则对**全部** LDAP/AD/RADIUS 账号永不命中，而策略页上它显示「已启用」。写值点收在 `finishExternalAuth` 里——那是口令认证源这条链的唯一出口，OIDC 的 `passwordAuthOf` 返回 nil，**结构上进不来**（拿不到口令的源不会误落一个假判定，恒 `unknown`）。三条纪律：**只落 `pw_strength` 一列，外部口令的明文与哈希都绝不入库**（外部账号 `pass_hash` 恒空是一道闸，也是 `guardLocalCredentialForAdmin` 判「是不是外部账号」的判据）；**每次登录都判**，不是只在建号那次（目录侧改口令白帝完全不知情）；**只收紧不放宽**——走完「先重置一个本地口令再提权」补救路径的账号同时持两把口令，用外部那把强口令把标记刷成 `strong`，等于让一把仍然可用的弱本地口令从策略视野里消失（纯外部账号 `pass_hash=''` 不受此限，双向如实更新，否则目录侧改强之后白帝这边永远清不掉 `weak`）。落库写失败只记日志不打断登录：口令是对的，用一次库写抖动把人挡在门外是明显更坏的方向。
...
knock 类（Go 侧固定前缀 `取敲门令牌失败：` / `SPA 拨号失败：` / `SPA 敲门包发送失败：`，清单在 TS 侧的
`KNOCK_FAIL_PREFIXES`，由一条跨轨守卫直接读 `dataplane.go` 抠出全部 `markKnockFail(` 前缀比对）err 清空即真恢复 →
收起；tunnel 类（其余一切，
...
### ✅ 控制中心信任锚：一份材料喂两条链路（安卓两半齐；**桌面端只有数据面那一半**）
...
**边界（不许多说一个字）**：
- **哪一端有锚入口，逐端说清**——这是本条最容易被读成「已经解决了」的地方。控制面 HTTPS 这一跳
  在每个端上都是**两条独立链路**（页面用宿主 webview 的信任库、数据面用 Go 的 `crypto/x509`），
  必须分开数：

  | 端 | 数据面（Go，取敲门令牌） | 页面（登录 / 该端 `lib/api.ts` 的每一次 fetch） | 自签部署下还要不要人工导证书 |
  |---|---|---|---|
  | 安卓 | ✅ `baidimobile.Config.ControlCaPEM`（构建期 `-PbaidiControlCa`） | ✅ 同一份字节生成的 NSC | **不要**——前提是那个包**构建时真传了** `-PbaidiControlCa`；不传的包照样能构建，两条链路都会栽（构建日志当面说明） |
  | 桌面 macOS / Windows / Linux | ✅ `baidi-tun -control-ca <PEM>` → `dataplane.Config.ControlTLS` | ❌ **没有入口**：WKWebView / WebView2 没有 NSC 等价物，只认系统信任库 | **要** |
  | iOS | ⚠️ 绑定层有 `ControlCaPEM`，**壳里没有写值方** | ❌ 没有入口 | 要，但目前没有可用姿态 |
  | 鸿蒙 | ⚠️ 同 iOS | ❌ 同 iOS | 同上 |

  所以**桌面端不是「收口了」**：`-control-ca` 消掉的只是「登录进得去、隧道却永远敲不开门」这半边的
  **单独**失败面（那半边此前连补的入口都没有，见 clients/BUILD.md 10.3e），页面那半边的唯一办法
  仍是把控制面证书导进本机信任库、或给控制面换一张受信证书。**文档里一律写「桌面端只有数据面
  那一半」，不许写「桌面端已收口」**——与「安卓已收口 ≠ 移动端已收口」是同一条纪律，同一族的错话
  本波已在 console/DESIGN.md 上栽过两次。`dataplane.Config.ControlTLS` 为 nil 时行为与改造前逐字一致，
  `-control-ca` 不传即 nil。
- **桌面端这一半的几个设计点**（都在 `clients/desktop/src-tauri/src/main.rs` 与
  `gateway/cmd/baidi-tun/controltrust.go`）：
  - 锚只认**约定路径** `<家目录>/.baidi/control-ca.pem`（Windows 是 `%USERPROFILE%\.baidi\control-ca.pem`）。
    **刻意不开环境变量入口**：桌面 app 从 Finder / 开始菜单双击启动时不继承 shell 环境，
    加一个 `BAIDI_CONTROL_CA` 就会造出「终端里 `npm run tauri dev` 生效、点图标启动静默不生效」
    两种现场完全同形的行为。锚**不能由控制面下发**（循环论证，同 JWT 公钥那条），只能带外放进去。
  - 四态 `Unknown` / `Absent` / `Ready` / `Unsafe`，**不许塌成 bool**：「家目录判不出来、压根没看过
    那个位置」与「确认没有锚」是两件事，塌成一个会让一台环境异常的机器被界面陈述成前者从未成立的结论。
  - **`Unsafe` 当场拒绝接入**，不是悄悄不用（`control_ca_arg` 返回 `Err`，有变异用例钉住）。
    判据：符号链接一律拒（lstat 与 root 打开它之间隔着用户输密码那几秒，链接目标可被换掉）、
    属主须是本人或 root、`0022` 位不可写、**父目录同判**（能在目录里把文件整个换掉是一回事）。
    理由是这份 PEM 的消费方是**提权后的 root 进程**：同机另一个普通用户若能写它，就能让数据面
    信任他签的证书 → 冒充控制面 → 换走敲门令牌，而两端日志全都正常。★**Windows 上这道复核弱于
    unix**（取不到 unix 属主与权限位，如实放行，保护退化成 `%USERPROFILE%` 的继承 ACL）。
  - **判据在本仓只有一份实现**：`knock.ControlTLSFromPEM`（文件入口 `ControlTLSFromFile`），
    桌面 `-control-ca` 与移动端 `ControlCaPEM` 共用它，必须同真同假——各写一份的话一端修了
    另一端零感知，而两侧的现场症状毫无共同点（正是本节开头那张表）。它的三条语义：空 → nil
    = 系统信任库；非空 → 系统池 **∪** 锚；**解析不出任何证书 / 文件读不到一律报错**，绝不静默
    回落成系统信任库（那就是「配了却不生效」，现场与"根本没配"完全同形）。锚是自签叶子还是
    CA 都能用（Go 允许把自签叶子直接当根），且**有效期与 SAN 照常校验**——这正是它比"证书指纹
    钉扎"强的地方，钉扎会连带把这两项一起关掉。
  - `baidi-tun` 启动期打一行**回执**（`loadControlTrust`），三种结局各一句话：没配 → 系统信任库、
    配了且 `-control` 是 https → 已装载、**配了但 `-control` 是 `http://` → 锚不生效且这一跳完全没有
    加密**（敲门令牌明文过网）。回执不是装饰：「以为配上了」与「真的生效了」在现场完全同形。
  - 覆盖范围在界面上有**两处**如实表述（加上面那行启动回执共三处），都有执行方：接入信息卡「控制中心信任」那一行
    （`Connect.vue` ← `tunnel.ts` 的 `controlCaSay` / `CONTROL_CA_SCOPE_NOTE`）、
    登录撞证书墙时的归因（`diagnose.ts` 的 `explainControlFailure`，给的是**导系统信任库**的具体命令，
    而不是「去放个锚文件」——方向说反了，用户会照着做完仍然登不进去）。
- **演示站现在挂的是受信证书，但那是部署姿态、不是产品能力。** 101.43.125.131 自 2026-09-08 起用的是
  Let's Encrypt 的 **IP 地址证书**（大陆机器上未备案域名会被 DNSPod 在 HTTP 层拦掉，域名路线签不下来；
  四条硬约束与 fail-safe 见 CLAUDE.md「坑」和 deploy/README.md「HTTPS 证书」）。它的全部意义是
  **那一台机器上**「导锚」这一步可以省掉；**产品侧一个字都没变**——换任何一台自签部署，上表的 ❌
  与「要导证书」原样成立。而且这张证书**只有 6 天 15 小时**（`shortlived` 是 IP 证书唯一可用的 profile），
  续不上时续期脚本会把 nginx 指回自签，**演示站的受信状态是会自己消失的**：拿它当「客户端已经不
  需要信任材料了」的证据，下一次长时间停机之后就会翻车。顺带，这也是「信任池 = 系统池 ∪ 锚」那条
  规则第一次真的派上用场——带旧自签锚的安卓包按设计不该掉线（LE 证书走系统池验），但**这一条本轮
  没有在安卓真机上复验**，别当成实测结论引用。
...
### ⚠️ 移动端原生壳（安卓 VpnService：2026-09-03 **一台**真机整链跑通过**一次**；iOS PacketTunnelProvider：源码级修复、**未实机**）

> ★标题几经修正，把口径一次写清。最早写「两端均未实机」（与本节正文自相矛盾）；随后改成
> 「分段验过、整链未通」——那一版停在「敲门取令牌因演示站自签证书失败」那一刻。**当天晚些时候
> 两个卡点都修了**（构建期注入控制中心信任锚 + 接入态改判健康行），同一台机器上把取令牌与业务流
> 两环补完，逐条证据见 `clients/BUILD.md` 12.4 的验收表（`ready:true healthKnock:true`、
> 设备侧 `nc 10.99.0.36 8080` 退出码 0、健康态翻 `healthTunnel:true`）。
> **但仍然不许写成「移动端数据面可用」**：证据是**一台设备、一次**（OPPO PKU110 / Android 16 / arm64、
> debug 包、对着演示站一个站点），没有第二台设备、没有 release 包、没有换网络环境复现；
> iOS 与鸿蒙壳那两端**一个真机数字都没有**。
...
★当日这条**曾**卡在下一环：取敲门令牌因演示站自签证书失败（`x509: certificate signed by unknown authority`），
且移动端 UI 判据仍停在「引擎启动成功即算接入」（同一时刻桥报 `stage:up` 而健康行是 `knock=false`）。
**两条当天都修了**，见上面「控制中心信任锚」与「移动端接入态判据」两节，验收记录在 `clients/BUILD.md` 12.4。
★**这条验收记录有一个前提已经变了，复验时必须注意**：那次是对着**自签证书**的演示站做的（起作用的正是构建期注入的锚）；
演示站自 2026-09-08 起换成了 Let's Encrypt 的 IP 地址证书，于是「锚真的生效」这件事**不再由日常访问顺带覆盖**：
当时那两道反例对照（不带 `-PbaidiControlCa` 构建、把 NSC 的 `<domain>` 改成另一个 IP）**现在也一起失效了**，
因为两道都依赖「系统信任库不认这张证书」这个前提。要复验只能换一台自签部署去验。
...
- **自伤 DNAT 在入口被拒，不是挂一条告警**（wave11 行动 13-①）。`store.NATWarnSPA` 是一条**常量字符串**，只要有启用中的策略就无条件挂出来，它并不知道这条规则做了什么。而 DNAT 有两种自伤形态、两种都是「入口 200 OK、规则照灌 prerouting、控制台全绿」：① **转换后目的 = 网关自己的监听口**——等于用地址转换给零信任接入面再开一扇门，而控制面对这扇门一无所知（剖面落点、七层入口主机名、网关页登记的对外接入地址、隐身回执里的"规则集保护端口"，全部按网关**自报的监听地址**算）；② **对外发布端 = 网关自己的监听口**——prerouting 的 dstnat 排在**路由决策之前**，命中的报文整条改道走掉，本机服务再也收不到，而网关到控制面的心跳是**出向**的、照常在线，症状是全员拨号超时且链路上零报错。判据是 `store.natSelfPublish`（纯函数，归一之后才跑——`TranslatedPort` 留空时由归一补成 `DstPort`，提到前面查就放过了最省事的那种写法），**端口取自网关心跳自报的 `proxy`/`spa`/`web`，不写死 18443/18201/18444**：写死的话一台 `-proxy` 换过端口的网关上，这道闸既放过真自伤、又拦下正常业务，两个方向同时错。取不到上报时退守出厂默认端口并在拒绝文案里当面写明「按默认端口保守判定，请等它心跳上报一次后重试」。形态照 `baidi-ipsec` 的 `parsePeer` 拒收 FQDN 的先例：拒绝要说得出**是哪个端口、它是干什么的、改错会出现什么症状**。
- **「攻击面 = 0」的前提集里有 DNAT**（wave11 行动 13-②）。一条**已确认灌进内核**的 DNAT 本身就是在网关的公网地址上开一个端口，而 SPA 隐身是 filter 表上按隧道口/敲门口写死的两条规则，与 nat 表无关。判据是网关回执 `applied` 而不是库里的 `enabled`（网关没带 `-nat` 时规则一条都不会进内核，算成敞口是另一个方向的假事实），判不出来的（离线 / 旧网关不上报 / 策略表读失败）**单列**且同样按下断言——不可判定既不算敞口，也不能拿去背书。整句断言的判定收在后端一处 `api.stealthClaim`（三条前提：隐身逐台实测 armed、无七层敞口、无 DNAT 敞口且判得出来），前端只渲染 `stealthClaimOk`：前提集分散在两条轨上时，加一条必然漏改其中一处，而漏改的那处恰好是整页最强的那句话。
...
- **回程路由与带宽两条配置侧风险仍靠文案提示**（`store.NATWarnRoute` / `NATWarnBandwidth`），控制面无从实测。SPA 互斥那条现在分成两半：**自伤的那半有真执行方**（入口拒绝，见上），而「被 DNAT 发布出去的普通业务端口在公网上可探测」这半仍是取舍本身、只能如实提示——它是 NAT 这个功能的用途，不是错误。
- **自伤闸只覆盖网关自己的三个监听口**。DNAT 把某个内网主机的高危端口（数据库、内网管理口）发布到公网，白帝一个字都不会说——那是管理员的授权决定，控制面没有"哪些内网端口不该公开"的判据，编一份出来就是又一个不可验证的断言。
- **闸的地址判据是网关上报的网卡地址与回环，不含"同网段即算"**。一台网关有未上报的地址（心跳那一瞬新加的网卡）时，指向它的自伤 DNAT 判不出来。放宽成按网段判会把正常发布内网业务的规则整批拦下——一道会误伤的闸最后一定被人整个关掉。
...
- **三条防线的聚合单位各不相同，且「不可判定」不塌成好值**（wave11 行动 12）。隐身防线按源 IP、账号防线按账号、终端防线按 `(账号, 设备指纹)`——改造前后两条都返回账号名且是同一批条目，「风险终端 TOP5」与「风险账号 TOP5」显示同一份数据、各起一个名字，看的人以为交叉印证了两次。风险档由 `store.RiskOfDisposal` **一处**折算（block/degrade→high、gray→low、allow→none、**认不出的值→unknown 而不是 none**），`api.riskOfAccount` 与本页共用它。`DefenseLine.Risk` 是三态指针：终端防线在「一台终端都没上报过环境」时下发 `null`，页面画「—」而不是 0——0 分在那套阈值下就是绿色的「良好」，而那恰恰是全新部署、客户端还没铺开时的常态形状。`DefenseLine.Unknown` 单列「判不出来的实体数」（账号线：从未上报过 posture 的账号；终端线：登记在授信台账却没有任何判定的终端），**TOP 为空时它是唯一能区分「面上很干净」与「没有判定材料」的数**。两个键都不许 `omitempty`（缺席会被前端补回 0，两种语义又同形）。聚合刻意不走 `PostureReports()`：那条查询带 `ListLimit=500` 上限却不告诉调用方被截断了，26 个账号 × 20 台就能撑满。
...
### ✅ 服务端版本身份（wave11 行动 18：从「源码常量 + git 哈希」到两个字段、构建期注入、未注入即如实说）

PRD FR-UPG-07/14。改造前**服务端版本号没有单一真相来源**，三处各说各的：

- **控制面**是 `const Version = "0.3.0"`。`-ldflags -X` **对常量静默无效**（Go 链接器只改可写字符串变量；实测：注入后二进制里仍是旧值、退出码 0、无任何告警）。于是「当前版本」与发布动作彻底脱钩——改一行 Go 源码才算发一版，构建脚本说了不算，而任何一个 `go run` 起来的进程也自称 0.3.0。
- **网关**是构建期注入的 **git 短哈希**（`deploy/build.sh` 里 `BAIDI_VERSION:-$(git rev-parse --short HEAD)`）。哈希 `ParseVersion` 必失败，而旧的 `gatewayConsistency` 把"解析不出来"与"解析出来了但确实不等于目标"合并成同一条 stale——**于是每一台按脚本装出来的网关都常年挂着「升级后版本将与控制面不一致，须同步升级」**，控制台那一栏恒黄，这条组件一致性校验退化成一句永远为真的告警（一条永远为真的告警等于没有告警，真正的不一致反而淹没在里面）。
- **主机上没有任何版本戳**：版本只活在心跳报文与控制台页面里，而排查现场第一件事恰恰是 ssh 上去看这台装的是什么——控制面挂了、或正在切换温备时，页面本身就是不可用的。

**形态：两个字段，都由构建期注入。**

| 字段 | 是什么 | 谁在用 |
|---|---|---|
| 语义版本 `x.y.z` | 发布方**声明**的东西，单一真相来源是仓库根的 `VERSION` 文件（发布动作 = 改它） | 升级判定：能不能升 / 是不是降级 / `minSource` / 组件一致性。**只有它能排序** |
| 构建标识 | 这一份二进制**到底**是哪次构建产出的（提交短哈希 · 构建时间） | 取证。同一个 `0.3.0` 可以被构建一百次，其中九十九次含着不同的代码（含 schema 迁移） |

合成一个字段就必然二选一，而改造前网关选的正是后者。控制面走 `control/internal/buildinfo`（`baidi-control` 与 `baidi-standby` 共用同一组注入，`deploy/build.sh` 用同一个 `CTL_LDFLAGS`——两者版本在构建那一步就分家的话，温备那条版本判定从源头起就是假的）；网关是另一个 module，注的是它 `main` 包里的三个变量。

- **未注入时如实说「未注入」，绝不回落到任何常量**。空 = 不可判定，是本项目对"探不到"的一贯处置。回落等于让一个**根本没经过发布流程**的二进制自称某个版本，而升级判定会一本正经地拿它去比大小。`CheckUpgrade` 对空当前版本 **fail-closed** 并给出可执行的理由（"这个二进制不是 deploy/build.sh 产出的交付件"，而不是"版本号写错了"——两句话会把人送去完全不同的地方）。
- **机读与展示两种形式分开**（`BuildID()` vs `BuildText()`）。后者在什么都不知道时吐「未注入」三个字，那是给人看的；一旦它被误用在上报/落库路径上，下游就再也分不出「对面说它不知道」与「对面报了一个叫『未注入』的构建号」。
- **注入值里不能有空格**：`-ldflags` 的内容按空格分词，`-X main.build=abc123 · 2026-…` 会让链接器**直接报 usage 退出**（实测，构建整个失败）。所以 commit 与 builtAt 各注一个变量，拼接放在 Go 里做。
- **构建期自检**：`-X` 打错包路径、或目标被改成 const，链接器都不报错。所以判据只能是**跑一遍产出的二进制看它自己报什么**——`build.sh` 另编一份本机架构的二进制跑 `-version`，语义版本与构建标识各验一次（只验一个的话，另一个哪天被写错也不会有人发现）。
- **主机上的版本戳补齐两条**：`$BD_PREFIX/VERSION`（`install-remote.sh` 装的），以及 `baidi-control -version` / `baidi-gateway -version` / `baidi-standby -version`。控制面那个是**一行 JSON**——它有机器消费方（备机要读它来回答"我这台机器上 baidi-control 是哪一版"），人话格式一改那条链就静默断了。
- **升级包的 `component` 字段参与判定了**。改造前它被解析、校验（必须是 `control|gateway`）、写进审计，然后就丢掉：无论包给谁，"当前版本"一律取控制面的。分离式部署下的后果**已实测复现**——控制面 0.4.0、网关还在 0.3.0，传一个 `component=gateway` 的 0.4.0 包，得到「升级包版本与当前运行版本相同（0.4.0），无需升级」并**阻断**，一次完全必要的网关升级被系统劝退，而理由里那个"当前运行版本"说的是另一个组件；方向相反的情形同样存在（控制面低、网关高时，同一个包被判成合法升级）。现在网关包**逐台判定**：任一台会被降级 → 阻断并点名（网关是数据面，起不来就是那一路业务全断）；全部已到位 → 阻断说"无需升级"；有的能升有的已到位 → 放行（滚动升级的正常中间态）；**一台都判不出来 → 阻断**（判不了就不该说"可以升"）；部分不可判定 → 放行但当面声明**结论不覆盖那几台**（一个悄悄缩小了范围的结论比没有结论更坏）。

**不能声称 / 刻意不做**：

- **控制台仍不替换二进制**。升级由部署脚本执行（`deploy.sh` 重跑即换版本并保留数据库），控制台只负责升级前的校验与备份——进程自替换与回滚未实现，谎称能一键升级的后果是升级中断时无人能恢复。
- **没有版本回滚的执行方**。`AllowDowngrade` 只是让校验放行，回滚动作与数据库结构的向下兼容都不存在。
- **`VERSION` 里的 `0.3.0` 不代表"这个产品已经发布过 0.3.0"**，它只是当前声明值；PRD 第 4 章那条版本链（2.1.1→2.1.5→2.1.12）是源产品的历史，白帝没有，故未照搬。
...
- **姿态随心跳上报，网关页逐台可见**（`tunnelIdWarnings`）。**三态各说各的**：`&false` 逐台点名并写清后果与关回的路；`nil`（旧网关根本不报）如实报「无从判断」——静默通过等于替一台按旧模型在跑的网关背书；`&true` 不出告警。网关侧**开着也报**，否则「已收口」与「旧网关不会报」在控制面看来完全一样，而这两者的运维动作恰好相反。
...
### ✅ 敲门链两端的归因订正（wave11 行动 11：FR-SEC-SPA-02 / FR-MON-04/05）

**① 时钟不准的正常终端不再被顶成「攻击源 TOP1」。** SPA 敲门信封有 ±30s 的时间戳窗口
（`spa.skew`），改造前超窗与「nonce 缺失 / nonce 重复」共用一个 secevent 类别 `knock-envelope`，
中文名逐字写着「敲门信封无效/重放」。而超窗**几乎唯一常见的成因是终端时钟偏差**：客户端保活
每 15s 敲一次、且每轮敲**全部**落点，一台偏了 31 秒的正常员工机一天稳定产出几千次超窗拒绝，
必然是安全概览「攻击源 TOP」的第一名——管理员照着面板去封的是自己的员工，而真正该做的
（给那台机器校时）在页面上一个字都没有。

现在拆成独立类别 `knock-clockskew`：`knock.ClockSkewError` 带**实测偏移量与方向**（正 = 包内
时间戳偏早，可能是时钟慢也可能是延迟到达的旧包；负 = 在未来，**只可能**是时钟快——重放者
手里是捕获的旧包，造不出未来时间戳），一路进日志与审计正文。它同时归入
`store.AttackExemptCats`：**拒绝照旧落审计**（那台机器确实连不上，这件事必须查得到），
只是不进「攻击源」这个带处置含义的口径。同 `proxy-capacity` / `knock-cache-full` 的处置——
**归因错的统计比没有统计更坏**。

**可接受的残留**（是取舍不是疏漏）：延迟超过 30s 才到的**被动重放**也落进这一类（nonce 去重
排在时间戳之后），因此也不进 TOP 面板。它即使信封过关也开不了窗（敲门令牌 90s 寿命 + jti
一次性），而近距离（≤30s）的被动重放走 `knock-envelope`（nonce 重复）、主动重放走
`knock-replay`（jti 重复），**两条真信号照常计入**。

**② 客户端不再把「UDP 包写出去了」说成「门敲开了」。**

- **写失败此前被整个吞掉**：`knockOne` 的收尾是 `if e == nil { if werr == nil { markKnock() } }`，
  封装失败与 `Write` 失败都不打日志、不写 `knockErr`、也不动 `knockOK`。于是发不出去的那一轮
  在健康行上与「什么都没发生」完全同形，界面停在上一次成功的 `knock=true err=-`。
  「UDP 的 Write 不会失败」是错觉——这是已连接 socket，内核会把上一次的 ICMP 端口不可达
  挂回来（下一次 Write 直接 ECONNREFUSED），本机路由不通时当场 `no route to host`，
  而这两种恰恰是「网关没在听 SPA 口」「路由到不了那台落点」的唯一线索。现在有执行方
  （新增敲门类前缀 `SPA 敲门包发送失败：`，跨轨清单见上文 `KNOCK_FAIL_PREFIXES`）。
- **「敲门包已发出」与「隧道真的通了」在界面上分成两件事**。桌面端接入信息卡此前：
  「加密隧道」恒是绿色写死的「已建立 · <密码学>」（`tunnel` 位可能一次都没翻过）、
  「SPA 敲门」恒是「已完成 · 已开放行窗口」。移动端判据在 wave10 已改对，**措辞这一半没改**
  （`knockView` 仍在 `healthKnock` 为真时说「已开放行窗口」）。现在两端统一：
  隧道那行按 `tunnel` 位三态（真拨通过 / 还没有业务流量 / 老数据面不报＝判不了），
  敲门那行只陈述到本机为止（「敲门包已发出 · 网关不回包，是否已放行以能否访问业务为准」）。

**★绝不改成让网关回包。** SPA 不应答是**正确的安全取舍**——一个应答就等于向扫描者确认这个
端口存在，隐身整个没了。所以「门开没开」在协议上就是客户端不可知的，能做的只有把话说准，
并把唯一能验证它的东西（真访问一次业务）写在界面上。
...
（配置面比实现更宽，用户照着配会撞墙）。
★**表里有一行已经不是缺口了**（闲置账号治理的策略化，wave9 同波补齐），**留行不删**：
删掉它，下一次全量扫描会把同一条当"从未表过态"再报一次，而这张表的存在意义正是让那件事不再发生。
判断某一行现在是什么状态，看该行开头的符号，不看本节标题里的"九项"。
...
| **闲置账号治理的策略化** | FR-MON-19（P1） | ✅ **已补齐（wave9 行动 7），本行留作留痕**。★这一行此前一直写着「⚠️ 只做了一半」，而修复在同一波就落了地——与 SCOPE ch5「闲置账号治理已补齐」并排读是两个相反的结论，wave11 行动 19 订正。当时缺的两项现在都有：策略落 `settings`（`store.IdlePolicy` = `thresholdDays` 7~3650 + `autoLock`，出厂 90 天且**不自动锁定**）、`GET/PUT /users/idle/policy`（读 = 任意管理员，写 = `PermSecurity`），执行方是 `StartIdleLockLoop`（`BAIDI_IDLE_LOCK_INTERVAL`，默认 1h；间隔只决定"多久查一次"，要不要锁由落库策略决定）。三条纪律见 CLAUDE.md：自动锁定与手工批量共用 `lockIdleAccount` 一条路径 · **永不处置管理员账号** · 管理员跳过不逐条记审计 |
...
**核实时点**：真实现 7 条、部分 19 条、未实现 8 条；**当前**：真 7 / 部分 20 / 未实现 7
——wave9 同批把 PERF-03 从 ❌ 改造成 ➖（外部认证预算 + 耗时埋点），计数跟着变。
（逐条表态与这两个计数的口径已写回 SCOPE ch20；**此处此前只留着"核实时点"那一组数字、
没跟上，与 SCOPE 并排读就是两个不同的结论**，wave11 行动 19 订正。）下面只摘出
...
### 🚫 wave11 收口：两段本波确认「不做」的边界（第三段是**指回上表**，不重复记账）

本波（行动 19）盘的是**记账**本身。三段边界里只有两段是真的没人说过，第三段（URL 级发布）
**上一张表已经逐字写过**——照 charter 原样再落一次就是重复记账，正是这条行动要消灭的东西，
所以这里只留一行指针。

| 项 | PRD | 性质与表态 |
|---|---|---|
| **L4 隧道的长命流只在建连时鉴权一次** | FR-INTRO-06（P0）/ FR-ARCH-02 | ⏸ **记档延后**（依赖本波行动 3 的身份链先落地，同波做会把 `proxy.handle` 改两次）。`proxy.handle` 对每条 TCP 流走**一次** `reg.Lookup` + `reg.Authorize`，随后就是两个方向的 `io.Copy` 直到某一端关闭——**此后不再复查任何判据**。于是四类"收权"对**已经建立**的隧道连接一律不生效，只影响下一条新连接：风险降权（`DenyUsers`）、JIT 授予到期、把人移出组织/用户组、资源 ACL 收窄。**别把它写成「L4 完全撤不掉」**：强制下线与账号封禁是例外，它们走 revoked 通道（`al.RevokeUser` + `proxy.KillUser`），逐条切断活跃连接并在回执里报"切断 N 条隧道"。**同一件事在 L7 上结论相反**——`webproxy` 每个请求重查一次 `resource.Authorize`（含 DenyUsers），101 升级后的 WebSocket 另有 `upgradeTracker` 周期复查 + 寿命不超过会话 Cookie，`upgraded.go` 的文件头注释还专门拿 L4 作正面对照。**表达面已经在替它说反话**：控制台资源页写着「high · 高敏（终端降权时暂停访问）」、门户磁贴写「终端降级 · 暂停访问」，而 `control/internal/api/api.go:1899` 的注释写着 JIT 到期后「网关下轮 `reg.Replace` 即失效（惰性回收）」——这三句在一条**开着不动的** SSH / 数据库 / RDP 会话上都不成立，而管理台与门户两边都显示已收权。修的时候形态照 `upgradeTracker` 复刻（周期复查 + 寿命上界），★**判据必须与建连那段同源**（L7 那边的注释写的是同一句）：各写一份必然分叉，而分叉的方向看不出来——放行侧多判一次没人发现，拒绝侧少判一次也没人发现 |
| **应用授权没有时间维度** | FR-AUTH-02（P1） | ⏸ **记档延后**。PRD 要的是「修改授权有效期：生效起止时间**精确到分**」（如 `2024-05-23 00:00 ~ 2024-06-30 23:59`）。白帝没有承载实体：`resources` 表只有 `allow_roles / allow_users / allow_groups / allow_orgs` 四个主体列，**一个时间列都没有**（`control/internal/store/sqlite.go:449`），静态 ACL 一律**永久**。唯一带时限的授权是 JIT（`jit_grants.expires_at`），但它是"申请 → 审批 → 临时开一扇门"那条路径，且 `store.MaxGrantTTL = 480` 分钟（上限与会话令牌 8h 对齐，注释写着「JIT 不该比一次登录还长」）——**它不是"这条授权 5/23 00:00 起、6/30 23:59 止"的实现**。★**对象库里那个「时间对象」不能读成"已经有一半"**：`store.TimeObject` 有表、有 CRUD、有种子，但 `control/internal/store/objects_usage.go:86` 写着 `n = 0 // 时间对象暂无落库消费者`，控制台对象库页也当面标注了「时间对象目前只是台账登记」——它一个判定点都没接上。实现位点是收敛的：主体清单补 `notBefore / notAfter` 两列（**回填空 = 不限**，绝不回填成"现在"——那等于升级那一刻全体授权同时失效；同 `backfillResourceSubjects` 回填 `[]`、`backfillBaselineScope` 回填 `'[]'`，**回填方向恒定为"不生效 / 不收窄"**）+ 判定收在 `store.SubjectIndex` / `handleGatewayPolicy` 展开那一处 + 剖面 `authorizeRes` 同构，**两处必须同真同假**，否则会出现「剖面里看得见、隧道里连不上」。★还有一条不能顺手做的：**网关侧不做时间推导**（它不知道"有效期"是什么），到期与否由控制面每轮下发时算好——与"组织展开成账号"是同一条纪律；反过来做，网关时钟不准就成了授权边界 |
| **URL 级发布** | FR-WEB-03（P0） | **不在这里重复表态**——上一张表（🚫 开放平台 API / SSO 代填 / URL 级发布）已逐字写过结论（记档延后）、后果（发布一个 Web 应用 = 把该 host:port 的整个后端暴露给被授权者）与实现位点。留这一行只为让"按 wave11 清单来找"的人落到那里，别再落一次表 |
